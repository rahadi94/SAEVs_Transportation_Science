...
from Deep_Reinforcement_Learning_Algorithms_with_PyTorch_master.agents.Base_Agent import Base_Agent
from Deep_Reinforcement_Learning_Algorithms_with_PyTorch_master.utilities.OU_Noise import OU_Noise
from Deep_Reinforcement_Learning_Algorithms_with_PyTorch_master.utilities.data_structures.Replay_Buffer import Replay_Buffer
...
from saev.rl.agents.DQN import epsilon_decay
from saev.rl.environment import convert_to_vector, convert_to_scalar
...
class SAC(Base_Agent):
    ...
    def __init__(self, config):
        ...
        if config.learnt_network:
            try:
                self.critic_local.load_state_dict(torch.load('result/critic'))
                self.critic_local.eval()
                self.critic_local_2.load_state_dict(torch.load('result/critic_2'))
        ...
        if config.learnt_network:
            try:
                self.actor_local.load_state_dict(torch.load('result/actor'))
        ...
        self.action_range = [config.environment.action_space.low, config.environment.action_space.high]
    ...
    def step(self, fleet, env):
        # """Runs an episode on the game, saving the experience and running a learning step if appropriate"""
        eval_ep = self.episode_number % TRAINING_EPISODES_PER_EVAL_EPISODE == 0 and self.do_evaluation_iterations
        self.episode_step_number_val = 0
        # while not self.done:
        self.action = self.pick_action(eval_ep, fleet)
        self.conduct_action(self.action, fleet, env)
    ...
    def checked_action(self, action, fleet, state):
        i = 0
        # print(action)
        action = convert_to_vector(action)
        # print(action)
        for vehicle in fleet.vehicles_to_decide:
            if vehicle.mode not in ['idle','parking','circling']:
                action[i] = 0
            if vehicle.mode in ['idle','parking','circling'] and vehicle.charge_state < 20:
                action[i] = 1
            if action[i] in [1,2,3]:
                if vehicle.charge_state > 90:
                    action[i] = 0
            if action[i] == 4:
                if len(vehicle.position.list_of_vehicles) <= vehicle.position.demand.iloc[0, state[0]]:
                    action[i] = 0
            i += 1
        # print(action)
        action = convert_to_scalar(action)
        # print(action)
        return action

    def pick_action(self, eval_ep, fleet, state=None):
        ...
        if eval_ep:
            ...
            a = random.random()
            if a <= epsilon_decay(fleet.episode):
                action = self.environment.action_space.sample()
            else:
        ...
        # action = [min(max(a, -1), 1) for a in action]
        # action = self.rescale_action(action)
        # action = [int(a) for a in action]
        action = self.checked_action(action, fleet, state)
        return action
    ...
    def save_models(self):
        torch.save(self.critic_local.state_dict(), 'result/critic')
        torch.save(self.critic_local_2.state_dict(), 'result/critic_2')
        torch.save(self.actor_local.state_dict(), 'result/actor')
